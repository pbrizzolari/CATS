...
def main():
    ...
    model_data = pd.read_csv(args.model_file, index_col=0)
    csf = linear_model.LogisticRegression(C=0.1,penalty="elasticnet", l1_ratio=0.1,n_jobs=-1,max_iter=1000,solver="saga")
    csf.fit(model_data, model_data.index)

    #getting the data
    data = pd.read_csv(args.input_file, sep="\t")
    index = sorted([852, 854, 855, 999, 1000, 1001, 1002, 1004, 1026, 2184, 2206, 2207, 2210, 2213, 2214, 849, 853, 2223,
